		...
		_ensureCustomFunctionsRegistered: function() {
			if (!jQuery.validator.xValFunctionsRegistered) {
				...
				jQuery.validator.addMethod("xVal_dateRange", function(value, element, params) {
					...
					if (params[0] != null) {
						if (parsedValue < params[0]) {
							return false;
						}
					}
					if (params[1] != null) {
						if (parsedValue > params[1]) {
							return false;
						}
					}
					return true;
					...
					var date = elem.value.toString().split('/');
					if (date.length < 2) {
						return xVal.Messages.DataType_Date || "Please enter a valid date in dd/mm/yyyy format."
					}
					var d = new Date(date[2], date[1] - 1, date[0], 0, 0, 0, 0);
					var parsedValue = Date.parse(d);
					if (isNaN(parsedValue)) {
